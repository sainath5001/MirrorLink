    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-white mb-2">
          Destination Chain Store
        </h1>
        <p className="text-gray-400">
          Mirrored Chainlink ETH/USD price on Base Sepolia
        </p>
      </div>
        ...
        <div className="space-y-4">
          <div>
            <label className="text-sm font-medium text-gray-400">
              Destination Price Store Address
            </label>
            <div className="flex items-center gap-2 mt-1">
              <code className="flex-1 font-mono text-sm text-white bg-gray-800 px-3 py-2 rounded">
                {CONTRACT_ADDRESSES.DESTINATION
                  ? formatAddress(CONTRACT_ADDRESSES.DESTINATION, 10)
                  : 'Not configured'}
              </code>
              {CONTRACT_ADDRESSES.DESTINATION && (
                <>
                  <CopyButton text={CONTRACT_ADDRESSES.DESTINATION} size="sm" />
                  <a
                    href={`${EXPLORER_URLS.BASE}/address/${CONTRACT_ADDRESSES.DESTINATION}`}
                    target="_blank"
